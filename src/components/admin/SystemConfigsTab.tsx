...
export default function SystemConfigsTab() {
  ...
  const [optimisticAllowGivePoint, setOptimisticAllowGivePoint] = useState<
    boolean | null
  >(null);
  const [optimisticShowLeaderboard, setOptimisticShowLeaderboard] = useState<
    boolean | null
  >(null);
  ...
  const actualAllowGivePoint =
    configs.find((c) => c.key === "ALLOW_GIVE_POINT")?.value === "true";
  const allowGivePoint =
    optimisticAllowGivePoint !== null
      ? optimisticAllowGivePoint
      : actualAllowGivePoint;

  const actualShowLeaderboard =
    configs.find((c) => c.key === "SHOW_LEADERBOARD")?.value === "true";
  const showLeaderboard =
    optimisticShowLeaderboard !== null
      ? optimisticShowLeaderboard
      : actualShowLeaderboard;
